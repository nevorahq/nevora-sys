...
| **I-13** | Low-severity billing hardening from the I-10 review (non-blocking): (a) re-validate input with zod inside `createCheckoutSessionForCurrentOrganization` (defense-in-depth; it is an exported Server Action), (b) drop the timestamp-less branch in `verifyBillingWebhookSignature` so a valid `t=`/`v1=` is always required, (c) reserve (not just assert) `storage_used_bytes` on upload to close the concurrent over-limit race. | Release owner | **OPEN (low)** |
