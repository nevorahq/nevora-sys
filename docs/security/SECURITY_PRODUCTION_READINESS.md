...
| Provider webhook signature is provider-agnostic HMAC, not a native Stripe/Paddle/LemonSqueezy verifier. | Correct while provider is unselected, but cannot be considered production-grade for a real provider. | When provider is selected, replace/extend adapter with native signature verification and event parsing for that provider. |
