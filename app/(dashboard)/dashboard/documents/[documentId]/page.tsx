  ...
  return <>
    <div className="mb-6">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
        <div><h1 className="text-2xl font-semibold text-text-primary">{document.title}</h1><p className="mt-1 text-sm text-text-muted">{DOCUMENT_TYPE_LABELS[document.doc_type]} · Updated {new Date(document.updated_at).toLocaleDateString()}</p></div>
        <div className="flex items-center gap-3"><span className="rounded-full bg-surface-sunken px-3 py-1 text-xs font-medium text-text-secondary">{DOCUMENT_STATUS_LABELS[document.status]}</span><DocumentDetailActions document={document} canUpdate={canDo(ctx, "data.write")} canDelete={canDo(ctx, "data.delete")} /></div>
      </div>
    </div>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      <aside className="space-y-4">
        {document.entity_type && <section className="soft-card-sm p-4"><p className="text-xs font-medium uppercase tracking-wide text-text-muted">Linked to</p><p className="mt-2 text-sm font-medium capitalize text-text-primary">{document.entity_type}</p></section>}
        {document.links.length > 0 && <section className="soft-card-sm p-4"><h2 className="flex items-center gap-2 text-sm font-semibold text-text-primary"><LinkIcon size={16} /> Links</h2><div className="mt-3 space-y-2">{document.links.map((link) => <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="block truncate text-sm text-text-secondary underline hover:text-text-primary">{link.title}</a>)}</div></section>}
      </aside>
    </div>
  </>;
}
