...
import { ArrowLeftIcon, CalendarIcon, TagIcon, WalletIcon } from "lucide-react";
...
export default async function TransactionDetailPage({ params }: PageProps<"/dashboard/money/[transactionId]">) {
  ...
  const { data: tx } = await supabase
    ...
    .select("id, title, amount, currency, transaction_date, type, note, status, account:money_accounts(name), category:money_categories(name)")
    .eq("id", transactionId)
      ...
      <div className="mb-6">
        ...
        <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">{tx.transaction_date}{account ? ` · ${account.name}` : ""}</p>
          </div>
