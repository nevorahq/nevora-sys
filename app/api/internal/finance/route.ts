...
import { createInProcessFinanceApplication } from "@/platform/finance/server";
import {
  resolveFinanceServiceContext,
  FinanceServiceIdentityError,
} from "@/platform/finance/service-context";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { ok: false, error: "Invalid Finance API request." },
      { status: 400, headers: RESPONSE_HEADERS },
    );
  }

  try {
    const write = isFinanceWriteOperation(parsed.data.operation);
    ...
    if (error instanceof FinanceServiceIdentityError) {
      return NextResponse.json(
        { ok: false, error: error.message, code: "SERVICE_IDENTITY_DENIED" },
        { status: error.httpStatus, headers: RESPONSE_HEADERS },
      );
    }
    logger.error("finance.http_transport.failed", {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      { ok: false, error: "Finance API request failed." },
...
async function authenticateServiceRequest(
  token: string,
  operation: FinanceHttpRequest["operation"],
  write: boolean,
) {
  const secret = process.env.FINANCE_SERVICE_AUTH_SECRET?.trim() ?? "";
  const verified = verifyFinanceServiceToken(token, secret, operation);
  if (!verified.ok) {
    const status = verified.reason === "misconfigured" ? 503 : 401;
    throw new FinanceServiceIdentityError(
      status === 503 ? "Finance service identity is not configured." : "Invalid service identity.",
      status,
    );
  }
  const requiredPermission = write ? "data.write" : "org.read";
  if (!verified.claims.permissions.includes(requiredPermission)) {
    throw new FinanceServiceIdentityError("Service identity lacks permission.");
  }
  const supabase = getServiceRoleClient();
  if (!supabase) {
    throw new FinanceServiceIdentityError("Finance service database access is not configured.", 503);
  }
  const currentContext = await resolveFinanceServiceContext(supabase, verified.claims);
