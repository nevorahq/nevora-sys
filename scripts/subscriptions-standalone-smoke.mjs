...
const subscriptionsBuild = resolve(repositoryRoot, "apps/subscriptions/.next");
const runtimeDirectory = resolve(subscriptionsBuild, "standalone/apps/subscriptions");
const runtimeEntry = resolve(runtimeDirectory, "server.js");

export async function runSubscriptionsStandaloneSmoke() {
  await requireFile(runtimeEntry, "Run `npm run build:subscriptions` before the standalone smoke test.");
  ...
  try {
    ...
    assert(health.body.service === "subscriptions", "health endpoint must identify as subscriptions");
    ...
    assert(root.status === 200 && html.includes("Subscriptions runtime"), "service page must render");
    ...
    const hidden = await fetch(`${origin}/api/internal/subscriptions`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ operation: "getSubscriptions", input: {} }),
      signal: AbortSignal.timeout(5_000),
    });
...
async function assembleStaticAssets() {
  const source = resolve(subscriptionsBuild, "static");
  const target = resolve(runtimeDirectory, ".next/static");
  await requireFile(source, "Subscriptions static build output is missing.");
...
async function waitForHealth(origin, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`Standalone Subscriptions process exited with code ${child.exitCode}.`);
    }
    try {
  ...
  throw new Error("Standalone Subscriptions runtime did not become healthy within 20 seconds.");
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`Subscriptions standalone smoke failed: ${message}.`);
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  runSubscriptionsStandaloneSmoke()
