...
export function DocumentDetailActions({ document, canUpdate, canDelete }: {
  ...
  return <>
    <div className="flex flex-wrap gap-2">
      ...
      {canDelete && <button type="button" onClick={() => setConfirmingDelete(true)} className="inline-flex items-center gap-2 rounded-lg border border-danger/30 px-3 py-2 text-sm font-medium text-danger hover:bg-danger-soft"><Trash2Icon size={16} /> Delete</button>}
    </div>
    ...
    {confirmingDelete && <div role="dialog" aria-modal="true" aria-label="Delete document" className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4"><div className="w-full max-w-md rounded-xl bg-surface p-5 shadow-xl"><h2 className="text-lg font-semibold text-text-primary">Delete document?</h2><p className="mt-2 text-sm text-text-muted">“{document.title}” will be removed from this organization. This can’t be undone from the dashboard.</p><div className="mt-5 flex justify-end gap-2"><button type="button" disabled={isDeleting} onClick={() => setConfirmingDelete(false)} className="rounded-lg px-3 py-2 text-sm font-medium text-text-secondary">Cancel</button><button type="button" disabled={isDeleting} onClick={remove} className="rounded-lg bg-danger px-3 py-2 text-sm font-medium text-white disabled:opacity-60">{isDeleting ? "Deleting…" : "Delete document"}</button></div></div></div>}
  </>;
}
