...
import { resolveFinanceServiceContext } from "./service-context";
...
describe("resolveFinanceServiceContext", () => {
  it("revalidates live tenant membership and rebuilds a minimal context", async () => {
    const context = await resolveFinanceServiceContext(serviceClient(), claims);
  ...
  it("fails when the signed actor no longer has an active membership", async () => {
    await expect(resolveFinanceServiceContext(serviceClient(null), claims)).rejects.toThrow(
