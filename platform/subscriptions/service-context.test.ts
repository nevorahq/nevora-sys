...
import type { SubscriptionsServiceClaims } from "@nevora/subscriptions-api";
import { resolveSubscriptionsServiceContext } from "./service-context";
...
describe("resolveSubscriptionsServiceContext", () => {
  it("revalidates live tenant membership and rebuilds a minimal context", async () => {
    const context = await resolveSubscriptionsServiceContext(serviceClient(), claims);
  ...
  it("fails when the signed actor no longer has an active membership", async () => {
    await expect(resolveSubscriptionsServiceContext(serviceClient(null), claims)).rejects.toThrow(
