...
export class SubscriptionsServiceIdentityError extends Error {
  readonly httpStatus: number;

  constructor(message: string, httpStatus = 403) {
    super(message);
    this.name = "SubscriptionsServiceIdentityError";
    this.httpStatus = httpStatus;
  }
}

/** Rebuild and verify a root-app context from already signed service claims. */
export async function resolveSubscriptionsServiceContext(
  ...
  if (!organization || !workspace || !membership) {
    throw new SubscriptionsServiceIdentityError("Service identity is no longer authorized.");
  }
