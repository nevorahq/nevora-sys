...
export class TasksServiceIdentityError extends Error {
  readonly httpStatus: number;

  constructor(message: string, httpStatus = 403) {
    super(message);
    this.name = "TasksServiceIdentityError";
    this.httpStatus = httpStatus;
  }
}

/** Rebuild and verify a root-app context from already signed service claims. */
export async function resolveTasksServiceContext(
  ...
  if (!organization || !workspace || !membership) {
    throw new TasksServiceIdentityError("Service identity is no longer authorized.");
  }
